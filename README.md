...
- 当前 README 作为产品外部快速上手落点，后续完整中文/英文手册将在后续 phase 补齐
...
- 请求级 Mock 模板
- 响应 HTML 预览
- MCP Server 调试与测试工作台（首版）

### MCP 首版能力边界

当前版本已经支持把 MCP Server 当作被测目标进行基础调试与回放：

- 独立的 MCP 请求模式
- `initialize`、`tools.list`、`tools.call` 三类结构化操作
- `tools.list` 返回 schema 后的参数表单映射与 raw JSON 回退
- MCP 响应结构视图、原始协议包查看、历史摘要与回放
- MCP over HTTP 执行链路

当前版本仍未实现：

- MCP over `stdio` 执行
- resources / prompts / notifications / sampling / roots 全量协议面
- conformance test suite 或多服务器编排
