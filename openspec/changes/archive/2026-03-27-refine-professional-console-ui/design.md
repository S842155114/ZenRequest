...
在第一轮主区域美化完成后，又出现了一个更具体的视觉问题：主工作区中的 sidebar、request、response 虽然各自已经更像独立的专业舱段，但它们之间仍然更接近“几张样式相近的卡片并排摆放”，而不是“同一台工作台设备中的对接模块”。这会削弱整体感，让用户在阅读界面时更容易把注意力放在单块面板，而不是整个控制台。
...
### Make the main workbench read as docked console segments instead of floating cards
主工作区会采用“共享底座 + 窄缝对接”的连接语言。sidebar、request、response 仍然保留各自独立壳体，但它们会坐落在同一个 workbench carrier 中，并通过更窄的技术缝、对齐的边缘和更克制的外轮廓相互连接，而不是以明显留白分开。

这项决策明确选择 `B 为主但更靠近 A` 的方向：不把三个区域焊成完全统一的大壳体，但也不保留松散的卡片摆放感。request 与 response 会更像同一主机体中的上下双舱，sidebar 则像左侧固定接入模块。

Alternatives considered:
- 完全一体式机架。Rejected，因为会过度削弱各区职责边界，也会让后续局部状态表达更难控制。
- 继续保留独立卡片，只做轻微间距压缩。Rejected，因为整体设备感提升有限，仍然会像三块并排面板。
- 强化连接桥和装饰接口。Rejected，因为容易过于造型化，抢走内容本身的注意力。
...
- [面板连接过强导致区域边界不清] → 保留每块面板的本地壳体，只收窄连接缝，不取消局部材质差异。
- [对接缝和 resize handle 处理不好会显得像 bug 而不是设计] → 让 handle 默认退到技术缝语义，仅在 hover / focus 时提升可见度。
...
4. 将主工作区调整为共享底座上的对接舱段，统一外层承载面、窄缝分隔和 handle 语义。
5. 收敛共享控件的反馈规则，确保按钮、tab、badge、dropdown 和 context menu 的优先级一致。
6. 通过组件测试与手动验证检查亮暗主题、桌面与紧凑布局、主操作反馈、面板连接关系和结果状态表达是否一致。
