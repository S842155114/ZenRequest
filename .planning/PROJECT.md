...
- 在现有单 server MCP 工作台基础上补齐 `sampling` 调试能力
- 保持本地优先、可检查、可回放的工作台体验，不把 scope 扩展到多 server 管理
- 后续 phase 必须先创建 dedicated phase branch，再执行 discuss / plan / execute

## Current Milestone: v1.3 MCP Sampling Debugging

**Goal:** 在现有单 server MCP 工作台基础上，补齐 `sampling` 的本地调试能力，让开发者能发起、查看并验证 sampling 相关交互，同时保持 ZenRequest 一贯的本地优先、可检查、可回放体验。

**Target features:**
- 在 MCP 工作台中支持 `sampling` 调试主链路
- 清晰展示 sampling 输入、响应与诊断信息
- 与现有 history / replay / diagnostics 体验保持一致
...
- [ ] 开发者可以调试 MCP `sampling` 能力，并检查其输入输出与安全边界
- [ ] `sampling` 调试结果应保留与现有 MCP 请求一致的 history / replay / diagnostics 体验
- [ ] 本 milestone 不扩展到多 MCP server 管理层，保持单 server 范围收敛
