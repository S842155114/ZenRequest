...
- ➡️ 当前正在定义 `v1.1 MCP Expansion`，聚焦 MCP 协议面扩展与 `stdio` 支持

## Current Milestone: v1.1 MCP Expansion

**Goal:** 把 ZenRequest 的 MCP 能力从当前 `initialize` / `tools.list` / `tools.call` 扩展到 `resources`、`prompts`、`roots`，并补齐 `stdio` 传输支持，同时保持可检查、可回放、可诊断的工作台体验。

**Target features:**
- MCP `resources` 调试与结果检查
- MCP `prompts` 调试与参数输入
- MCP `roots` 配置与请求支持
- MCP over `stdio`
- 单 server 工作台下的统一历史、回放与诊断语义
...
- [ ] 扩展 MCP 工作台到 `resources` / `prompts` / `roots`
- [ ] 为 MCP 工作台补齐 `stdio` 传输支持
- [ ] 继续保持单 server 场景下可检查、可回放、可诊断的统一体验
...
---
*Last updated: 2026-04-07 after v1.1 milestone kickoff*
